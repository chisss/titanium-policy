...
- 目录版本 `2026.08.28.1` 已开放投保人手机号、主险保额及受益人姓名/类型/份额真实执行；其余字段可提案但继续标记为未支持执行。
...
- 受益人集合字段使用 32 位稳定对象 ID；生效时按同一对象合并姓名、类型和份额，校验同顺位份额合计 100%，并由批改事件重建受益人读模型。
...
- 当前正式保全应用支持 `IMMEDIATE` 和四种已到期计划时态、投保人手机号、主险保额、受益人姓名/类型/份额及暂停、
  恢复、复效和终止状态动作；追溯时态、未来版本预写和其他字段执行器尚未开放。
...
### 2026-08-28 - 发布受益人集合字段正式保全

字段目录开放受益人姓名、类型和份额执行能力；权威快照与读模型统一使用稳定对象 ID，正式应用校验集合完整性和份额守恒，
批改投影在同一事务中重建受益人查询视图。
