...
/**
 * 投保意向单读模型投影映射器（MapStruct，事件 → 读模型字段拷贝）
 * <p>
 * 承接意向单"新建型"投影（{@link ProposalCreatedEvent} → {@link ProposalView}）的字段映射，取代投影处理器
...
public interface ProposalViewMapper {

    /**
     * 意向单创建事件 → 意向单读模型（就地 upsert）。
     * <p>
