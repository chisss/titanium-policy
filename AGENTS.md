...
- [ ] 🔴 **javadoc 承诺的机制是否真实存在**：注释里「由 XX 定时任务补齐」「待 YY 通道驱动」若指向一个不存在的组件，该链路在**该分支上是静默断裂**的——异常被 catch 吞掉、只留一行 info 日志，编译与测试全绿（m12-1501 实例：三处 javadoc 承诺的定时激活任务全仓不存在，起期在未来的保单永远停待生效）。排查链路时须把**注释里的承诺**当待验证项，反向搜索其指代物。
- [ ] **调度器不复制业务规则**：`application/scheduled` 只做「到期即触发」，能否执行由聚合前置校验判定；**不在读侧查询里过滤业务条件**（如能否激活、保费是否收讫），否则读侧规则与写侧漂移。
- [ ] **跨租户扫描**：批处理查询不带 `tenantId`（跨租户游标翻页），逐条以**行自带的 tenantId** 定位租户，不依赖线程上下文。样板见 `PolicyActivationScheduler`/`PolicyMaturityScheduler`/`AnnuityPayoutScheduler`。
