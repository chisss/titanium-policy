...
3. ⚠️ **Kafka 仅发布 3 个事件**：`KafkaEventPublisher` 只外发 `PolicyCreatedEvent`、`PolicyActivatedEvent`、`PolicyIssuedEvent`，其余事件不出域。下游若依赖保单状态需补充发布。
   - 🔴 **`policy.created` 已判定不应接 billing 开单（2026-09-14 m6-906）**：首期保费账单的权威路径是本域出单用例内**同步**开立——`IssuanceOrchestrator`/`IssuanceSaga` 创建保单后立即 `PremiumCollectionOrchestrator.collect` → `BillingServicePort.createPremiumBill`（Feign），
     账单结果是该用例的必需输入（建支付单/标记收讫/决定能否激活）。据创建事件在 billing 侧异步再开将**重复计费**。判定依据见 [跨域事件目录 §六.14](../docs/技术文档/跨域事件目录-2026-09.md)，登记点见 `KafkaEventPublisher.handlePolicyCreatedEvent`。
...
11. ✅ **存量测试编译问题已闭环（m1-801 实读复核，2026-09-11）**：原记录称 `IssuanceSagaTest`/`IssuanceOrchestratorTest` 引用「已不存在的 `ConfirmedPremiumRequestValidator`」——**该表述与代码不符**：类一直存在（分包改造时移入 `orchestration/issuance/validator/` 子包），错的是两个测试里**少了 `.validator` 一段的过时全限定名**。现均已修正（`IssuanceSagaTest` 改走 import 消除 FQN，符合「禁全限定名」规约），实测 `IssuanceSagaTest` 9 例 + `IssuanceOrchestratorTest` 9 例全绿、`titanium-policy-application` 25 个测试源文件编译通过、全仓门禁 **无需** `-Dmaven.test.skip=true`。教训：**测试源码里的全限定名不受 IDE 重构保护**，分包改造后必须全仓扫过时 FQN。
12. ✅ **保单现金价值只读查询能力（m0-717，2026-09-11）**：新增对外契约 `PolicyCashValueApi`（`GET /api/v1/policies/{policyId}/cash-value`，`contextId=policyCashValueApi`），调用方（计费域垫缴判定）只需保单号。链路：`PolicyCashValueApiProvider` → `PolicyCashValueQueryAppService`（application/query，读 `PolicyView` 取 productId/totalPremium/startDate，保单年度 = 生效日至估值日年差 + 1）→ `PolicyCashValuePort`（`port/product`）→ `PolicyCashValueAdapter`（infrastructure/adapter/product）→ product 域只读现金价值端点。
...
17. ✅ **包名拆分已完成：`domain/port`（m6-910）、`infrastructure/adapter` 与 `valueobject`（m6-911），2026-09-14**：本域曾有「**目录已拆、`package` 未拆**」缺陷（m6-908 侦察命中）——文件置于 `billing`/`clause`/`customer`/`investment`/`payment`/`product`/`ruleengine`/`underwriting` 八个子目录，**但 12 个端口类中 11 个的 `package` 声明仍是扁平的 `com.titanium.policy.port`**。ArchUnit 断言基于**字节码包名**而非目录树，故根规约 §3.4.13 在本域形同虚设（`git mv` 只搬文件、不换包名），后续新增端口照抄相邻文件会持续复制该缺陷而**编译通过、测试全绿**。
    ...
    - ✅ **同类残余一并清零（m6-911，2026-09-14）**：m6-910 全域扫描发现的另两处同类缺陷已整改——`infrastructure/adapter/**` **18 处**（11 main + 7 test）`package` 由扁平 `com.titanium.policy.infrastructure.adapter` 归位到 8 个对端域子包（`billing`/`clause`/`customer`/`investment`/`payment`/`product`/`ruleengine`/`underwriting`）；`valueobject/{issuance,policy}/**` **10 处**（8 main + 2 test）由扁平 `com.titanium.policy.valueobject` 归位到 `issuance`/`policy` 子包。
    - **验收（m6-911）**：`PolicyArchitectureTest` `@Override` 启用 `adapterShouldNotContainFlatClasses`（本域第 8 条启用的断言），架构测试 **43 例 0 失败（11 跳过）**、policy 域 **392 例 0 失败**；反向验证——向 `infrastructure/adapter` **顶层**注入 `FlatProbeAdapter` 探针后断言立即 RED（失败信息精确指向该类），删探针 `clean` 重跑复绿。探针**必须是新增顶层类**，不可把现有适配器的 `package` 改回扁平来验证（那会先让引用方编译失败，得到编译错而非断言失败）。
