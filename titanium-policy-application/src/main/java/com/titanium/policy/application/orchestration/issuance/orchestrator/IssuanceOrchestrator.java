 ...
 * </p>
 * <ul>
 * <li><b>一步出单</b>：单聚合、同步、即时返回——直接创建正式保单（含完整险种段与条款责任快照）， 编排在此完结；</li>
 * <li><b>两步/三步出单</b>：跨聚合、含核保跨服务、长周期——本编排器只发<b>起点命令</b> （创建投保单 / 意向单），随后的「核保 →
 * 承保 → 出单」全程由 {@code IssuanceSaga} 事件驱动接力。</li>
 * </ul>
 * <p>
 * 🔴 <b>本次改造要点</b>：
 * </p>
 ...
 * {@link PolicyProductAssembler} 装配），取代改造前的 {@code List.of(productCode)}
 * 裸编码；</li>
 * <li><b>参与方不再丢弃</b>：一步出单此前向命令传 null 参与方，现完整透传；</li>
 * <li><b>保额与缴费贯通</b>：改造前两步出单传 null 的 {@code sumInsured}/{@code paymentMode} 使
 * Saga 的真实保费计算被静默跳过，现由段承载真实值；</li>
 * <li><b>收费与渠道落地</b>：收费方式与渠道信息随命令进入保单；</li>
 * <li><b>命令装配内聚</b>：三支路的命令构造下沉至 {@code CreateXxxCommandAssembler}（红线 18），
 * 拒绝原因以「错误码 + 参数」承载（红线 15/16）。</li>
 * </ol>
 */
@Slf4j
@Service
...
public class IssuanceOrchestrator {
    ...
    /** 保费确认默认保单年度（首年） */
    private static final int DEFAULT_POLICY_YEAR = 1;
    /** 一步出单（免核保）无核保调整，确认保费时不纳入核保调整 */
    private static final boolean INCLUDE_UNDERWRITING_ADJUSTMENTS = false;
    ...
    private final CreateProposalCommandAssembler          createProposalCommandAssembler;
    private final CreatePolicyDirectlyCommandAssembler    createPolicyDirectlyCommandAssembler;
    private final CreateInsuranceDirectlyCommandAssembler createInsuranceDirectlyCommandAssembler;
    ...
    public IssuanceResult orchestrate(IssuanceRequest request) {
        ...
        if (mainProductId == null) {
            return IssuanceResult.rejected(request.bizNo(),
                    RuleDecision.rejected(PolicyErrorCode.ISSUANCE_MAIN_PRODUCT_MISSING));
        }
        IssuanceMode mode = productServicePort.getIssuanceMode(mainProductId, request.tenantId());
        log.info("产品驱动出单: bizNo={}, 主险产品={}, 由产品配置决定出单模式={}", request.bizNo(), mainProductId, mode);
    ...
    public IssuanceResult orchestrate(IssuanceProcessConfig config, IssuanceRequest request) {
        ...
        if (request.isSplitStrategy()) {
            return IssuanceResult.rejected(request.bizNo(),
                    RuleDecision.rejected(PolicyErrorCode.ISSUANCE_STRATEGY_UNSUPPORTED));
        }
        ...
        for (RiskAssessmentStep step : config.riskAssessmentSteps()) {
            if (!riskAssessmentExecutor.execute(step, request)) {
                ...
                return IssuanceResult.rejected(request.bizNo(),
                        RuleDecision.rejected(PolicyErrorCode.ISSUANCE_RISK_REJECTED, step.getName()));
            }
        }
        ...
        return switch (config.issuanceMode()) {
            ...
            case CUSTOM ->
                IssuanceResult.rejected(request.bizNo(),
                        RuleDecision.rejected(PolicyErrorCode.ISSUANCE_MODE_UNSUPPORTED));
        };
    }
     ...
     * <p>
     * 装配完整险种段（含条款与责任快照），使一步出单产出的保单与两步/三步出单结构一致—— 下游读侧与理赔无需区分出单模式。
     * </p>
     */
    private IssuanceResult executeOneStep(IssuanceRequest request) {
        ...
        ConfirmationSummary confirmation = premiumConfirmationService.confirm(insuranceLineAssembler.assemble(request),
                request.insuredPartyList(), request.bizNo(), request.bizNo(), request.periodStart(), request.tenantId(),
                request.channelId(), DEFAULT_POLICY_YEAR, INCLUDE_UNDERWRITING_ADJUSTMENTS);
        List<PolicyProduct> lines = policyProductAssembler.assembleFromInsuranceLines(confirmation.lines(),
                request.tenantId(), confirmation.calculationReferences());
        Money totalPremium = confirmation.totalPremium();

        commandGateway.sendAndWait(createPolicyDirectlyCommandAssembler.assemble(request, policyId, policyNo, lines,
                totalPremium, resolvePolicyForm(request), buildPolicyPeriod(request)));
        IssuanceResult result = IssuanceResult.policyIssued(request.bizNo(),
                new IssuanceResult.IssuedPolicy(policyId, policyNo, PolicyStatusCode.NOT_EFFECTIVE.getCode(),
                        lines.size(), totalPremium),
                totalPremium);
        try {
            IssuancePlanLine mainPlan = request.mainLine();
            CollectionResult collection = premiumCollectionOrchestrator.collect(policyId, request.holderCustomerId(),
                    totalPremium, request.collectionMode(),
            ...
            premiumScheduleOrchestrator.generate(policyId, collection.billId(), collection.billingAccountId(),
                    mainPlan != null && mainPlan.paymentFrequency() != null ? mainPlan.paymentFrequency().getCode()
                            : null,
            ...
            log.info("一步出单收费完成: bizNo={}, policyId={}, status={}, billId={}, paymentOrderId={}", request.bizNo(),
                    policyId, collection.status(), collection.billId(), collection.paymentOrderId());
        } catch (RuntimeException exception) {
            // 承保事实已经成立，收费失败不销毁保单；保单保持未生效，后续由补偿流程重试收费。
            log.error("一步出单收费失败（保单保留为未生效）: bizNo={}, policyId={}", request.bizNo(), policyId, exception);
        }
        log.info("一步出单完成: bizNo={}, policyId={}, policyNo={}, 险种段数={}", request.bizNo(), policyId, policyNo,
                lines.size());
        return result;
    }
     ...
     * 两步出单起点：创建投保单。
     * <p>
     * 只发起点命令；投保单创建产生 {@code InsuranceCreatedEvent} 后，「核保 → 承保 → 出单」 全程由
     * {@code IssuanceSaga} 事件驱动接力。
     * </p>
     */
    private IssuanceResult executeTwoStep(IssuanceRequest request) {
        ...
        commandGateway.sendAndWait(createInsuranceDirectlyCommandAssembler.assemble(request, insuranceId, insuranceNo,
                lines, resolvePolicyForm(request), mainPlan));
        ...
        try {
            ...
            throw new IssuanceOrchestrationException(PolicyErrorCode.ISSUANCE_UNDERWRITING_SUBMIT_FAILED,
                    partialResult, exception);
        }
        log.info("两步出单 - 投保单创建完成: bizNo={}, insuranceId={}, 险种段数={}；后续核保/承保/出单由 IssuanceSaga 接力", request.bizNo(),
                insuranceId, lines.size());
     ...
     * 三步出单起点：创建意向单。
     * <p>
     * 意向段（轻量：意向产品 + 意向保额）随命令落地；意向单提交后由 {@code ProposalIssuanceSaga} 自动转投保单，再由
     * {@code IssuanceSaga} 接力核保与承保。
     * </p>
     */
    private IssuanceResult executeThreeStep(IssuanceRequest request) {
        ...
        commandGateway.sendAndWait(createProposalCommandAssembler.assemble(request, proposalId, proposalNo,
                proposalLines, resolvePolicyForm(request)));
        IssuanceResult partialResult = IssuanceResult.proposalCreated(request.bizNo(), proposalId, proposalNo);
        try {
            commandGateway.sendAndWait(new SubmitProposalCommand(proposalId, PolicyConstants.ISSUANCE_AUTO_SUBMIT_REASON,
                    request.tenantId()));
        } catch (RuntimeException exception) {
            throw new IssuanceOrchestrationException(PolicyErrorCode.ISSUANCE_PROPOSAL_SUBMIT_FAILED, partialResult,
                    exception);
        }
        log.info("三步出单 - 意向单创建完成: bizNo={}, proposalId={}, proposalNo={}", request.bizNo(), proposalId, proposalNo);
     ...
     */
    private PolicyForm resolvePolicyForm(IssuanceRequest request) {
        if (request.policyForm() != null) {
            return request.policyForm();
        }
        ProductIssueRules rules = productServicePort.getIssueRules(request.mainProductId(), request.tenantId());
    ...
    private PolicyPeriod buildPolicyPeriod(IssuanceRequest request) {
        ProductIssueRules rules = productServicePort.getIssueRules(request.mainProductId(), request.tenantId());
        // ProductIssueRules 的等待期/犹豫期是可选配置，保持 Integer 以便 PolicyPeriod.of 做空值归零。
        Integer waitingPeriodDays = rules != null ? rules.waitingPeriodDays() : null;
        Integer hesitationPeriodDays = rules != null ? rules.hesitationPeriodDays() : null;
        return PolicyPeriod.of(request.periodStart(), request.periodEnd(), waitingPeriodDays, hesitationPeriodDays);
    }
    ...
    private void activateIfPeriodStarted(String policyId, String tenantId) {
        try {
            ...
            log.info("一步出单保单暂不可生效，等待保障起期: policyId={}, 原因={}", policyId, exception.getMessage());
        }
    }
}
